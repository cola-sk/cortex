...
<p align="center">
  <img src="public/screenshot-main.png" alt="Cortex 多智能体 CI 引擎 — 可视化流水线执行，支持并行 Worker、质量门禁和实时流式输出" width="780">
</p>

## 概述

Cortex 将多个 AI 智能体（Claude、Codex、Antigravity、Copilot、本地模型、自定义 OpenAI 兼容 API）编排为可配置的流水线。每条流水线是一个有向无环图（DAG），任务可串行或并行执行，并支持质量门禁决策点。

**核心能力：**

- **模型中心** — 自动检测本地 CLI 工具（Claude Code、Codex、Antigravity、Copilot），连接任意 OpenAI 兼容 API 或自建模型
...
**执行审查与拓扑图谱** — 查看任务输出、智能体迭代轨迹与人工决策，并配合实时的流水线 DAG 拓扑可视化。

<p align="center">
  <img src="public/screenshot-review.png" alt="Cortex 执行审查面板：拓扑图谱与人机协同决策" width="780">
</p>

**执行动作时间线** — 顺序列出智能体思维轨迹、工具调用事件、输出快照以及人工交互记录，全面掌握每轮执行的微观细节。

<p align="center">
  <img src="public/action-timeline.png" alt="按时间顺序排列的工具输出和人工审查反馈时间线" width="780">
</p>

**可视化流水线构建器** — 设计支持顺序依赖链、并行工作者以及条件决策门禁的复杂任务图。

<p align="center">
  <img src="public/screenshot-pipeline.png" alt="流水线编辑器：plan → implement → review 工作流" width="780">
</p>

**模型连接** — 自动检测本地 CLI 工具，便捷管理外部 API 提供商。

<p align="center">
  <img src="public/screenshot-models.png" alt="模型连接页面：已导入的 CLI 工具与 API 提供商" width="780">
</p>

**角色智能体** — 按角色（编排者、工作者、审查者、决策者）定义智能体专长，绑定相应的模型连接并配置独立的系统提示词。
...
└───────────────────────────────────────────────────────────┘
          ...
          ┌────────────┼────────────┐
          ▼            ▼            ▼
     Claude API  OpenAI 兼容   CLI (claude, codex, antigravity, copilot)
```
...
CLI 提供商通过 `@sking7/agent-cli-unified` 将不同 CLI 的 JSON/JSONL 流统一为同一套工具时间线。Claude Code、Codex、Antigravity、Copilot 的工具调用都会保存为 `tool_use` / `tool_result` 事件。
